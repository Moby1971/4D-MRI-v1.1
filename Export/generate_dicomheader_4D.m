function dicom_header = generate_dicomheader_4D(parameters,i,j,dimx,dimy,dimz,dcmid)
...
dcmhead.AcquisitionMatrix = uint16([dimz 0 0 dimy])';
...
dcmhead.InstanceNumber = (i-1)*parameters.NO_SLICES+j;          
dcmhead.ImagePositionPatient = [-(aspectratio*parameters.FOV/2), -parameters.FOV/2 (j-round(parameters.NO_SLICES/2))*(parameters.SLICE_SEPARATION/parameters.SLICE_INTERLEAVE)]';
...
dcmhead.SliceLocation = (j-round(parameters.NO_SLICES/2))*(parameters.SLICE_SEPARATION/parameters.SLICE_INTERLEAVE);
...
dcmhead.PixelSpacing = [pixelz pixely]';
...
dcmhead.CardiacNumberOfImages = parameters.nr_frames;
